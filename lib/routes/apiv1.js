...
function handleBinaryData(details) {
  var ret = {
    "b64" : details.value.toString("base64")
  };
...
function getKeyDetailsString (key, redisConnection, res, next) {
  redisConnection.get(new Buffer(key), function (err, val) {
    if (err) {
    ...
    var details = {
      key: key,
      type: 'string',
      value: config.redisValueToString({key : key, type : 'string', value : val})
    };
...
function getKeyDetailsList (key, redisConnection, req, res, next) {
  ...
  redisConnection.lrange(new Buffer(key), startIdx, endIdx, function (err, items) {
    if (err) {
    ...
    redisConnection.llen(key, function (err, length) {
      ...
      details.items = config.redisValueToString({key : key, type : 'list', value : items});
...
function getKeyDetailsHash (key, redisConnection, res, next) {
  redisConnection.hgetall(new Buffer(key), function (err, fieldsAndValues) {
    ...
    var details = {
      key: key,
      type: 'hash',
      data: config.redisValueToString({ key : key, type : 'hash', value : fieldsAndValues })
    };
    myutil.encodeHTMLEntities(JSON.stringify(details), function (string) {
      res.send(string);
    })
...
function getKeyDetailsSet (key, redisConnection, res, next) {
  redisConnection.smembers(new Buffer(key), function (err, members) {
    if (err) {
    ...
    details.members = config.redisValueToString({ key : key, type : 'set', value : members});
...
function getKeyDetailsZSet (key, redisConnection, req, res, next) {
  ...
  var endIdx = startIdx + 19;
  redisConnection.zrange(new Buffer(key), startIdx, endIdx, 'WITHSCORES', function (err, items) {
    if (err) {
    ...
    redisConnection.zcount(key, "-inf", "+inf", function (err, length) {
      ...
      details.items = config.redisValueToString({key : key, type : 'zset', value : items});
      myutil.encodeHTMLEntities(JSON.stringify(details), function (string) {
        res.send(string);
      })
    });
