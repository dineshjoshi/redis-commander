...
function getKeyDetailsList (key, redisConnection, req, res, next) {
  ...
  redisConnection.lrange(new Buffer(key), startIdx, endIdx, function (err, items) {
    ...
    items = items.map(function (item) {
      return {
        number: i++,
        value: config.redisValueToString({key : key, type : 'list', value : item})
      }
    });
...
function mapZSetItems (key, items) {
  var results = [];
  for (var i = 0; i < items.length; i += 2) {
    results.push({
      score: items[i+1].toString(),
      value: config.redisValueToString({key : key, type : 'zset', value : items[i]})
    });
  }
  return results;
}
